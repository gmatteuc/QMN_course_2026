...
Agreed while cleaning up the repository in August 2026. They exist so that a reviewer can open anyone's notebook and have it run, and so that the student-facing repository can be generated by a script rather than by hand.

**Notebooks.** One folder, `notebooks/`, one file per week, named `NN_topic[_vN][_TA|_student].ipynb`, lowercase, no spaces. `_TA` carries the solutions, `_student` is the version handed out, `_vN` marks a competing version while the team decides. Superseded material goes to `notebooks/legacy/`, never deleted and never left next to the live files.

**Environment.** One environment for the whole course, `qmn`, built from `environment.yml`, registered as the kernel `Python (qmn)`. A notebook that needs a new package adds it to `environment.yml` **and** to `REQUIRED` in `src/check_env.py`, so that everyone finds out by running the check rather than by hitting an `ImportError` mid-session. Never `pip install` from inside a notebook: the environment file is the single place where dependencies are declared.

**Data.** One copy of each dataset, in `notebooks/data/`, with its data dictionary beside it. Read it through the root idiom shown above, never with an absolute path, and never by downloading at runtime: the repository becomes private before the course starts, so any URL pointing at it will break. Datasets no longer used by a live notebook move to `notebooks/data/legacy/`.
...
**Outputs.** Notebooks are committed **with** their outputs while the course is being written, so that a reviewer can read the results without running anything. The `nbstripout` filter is therefore off; do not install it. Outputs are stripped once, at the end, by the script that generates the student repository.
